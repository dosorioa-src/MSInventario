        ...
        <div class="card">
          <div class="card-header" style="padding: 20px 40px !important">
            ...
            <div class="row">
              <div class="col-md-4 col-sm-12">
                ...
                <div class="input-group input-group-sm">
                  ...
                  <input class="form-control" type="text" v-model="buscador" @keyup="buscarUsuario" placeholder="Nombre de usuario...">
                </div>
              </div>
              ...
              <div class="col-md-5 col-sm-12 align-self-end text-end">
                <button
                  @click="btn_update_active=0, modal_title='Nuevo Usuario'"
                    ...
                    <form v-on:submit.prevent>
                      ...
                      <div class="row g-3">
                        ...
                        <div class="col-md-6 mb-3">
                          ...
                          <template v-if="$v.user.role_id.$error">
                            <p
                              class="errorMessage error"
                              v-if="!$v.user.role_id.required"
                            >
                              Este campo es obligatorio(*)
                            </p>
                          </template>                          
                      ...
                      <div class="row g-3">
                        <div class="col-md-6 mb-3">
                          ...
                          <select
                            class="form-select"
                            v-model="user.type_indetification_id"
                            @blur="$v.user.type_indetification_id.$touch()"
                          >
                            <option selected="" disabled="" value="">
                              Seleccionar
                            </option>
                          ...
                          <template v-if="$v.user.type_indetification_id.$error">
                            <p
                              class="errorMessage error"
                              v-if="!$v.user.type_indetification_id.required"
                            >
                              Este campo es obligatorio(*)
                            </p>
                          </template>                            
                        ...
                        <div class="col-md-6 mb-3">
                          ...
                          <input
                            class="form-control"
                            type="text"
                            v-model="user.number_identification"
                            placeholder="42455155"
                            @blur="$v.user.number_identification.$touch()"
                          />
                          <template v-if="$v.user.number_identification.$error">
                            <p
                              class="errorMessage error"
                              v-if="!$v.user.number_identification.required"
                            >
                              Este campo es obligatorio(*)
                            </p>
                          </template>                            
                      ...
                      <!-- <div class="row g-3">
                        <div class="col-md-6">
                          <label class="form-label" for="">Fotografía</label>
              ...
              <div class="table-responsive style-1 table-hover non-hover">
                ...
                <tbody>
                  <tr v-for="item in users.data" :key="item.id">
                    ...
                    <td>
                      <!-- <button class="btn btn-light btn-xs" type="button" title="" data-bs-original-title="btn btn-light btn-sm" data-original-title="btn btn-primary btn-sm">
                      ...
                      <div class="btn-group">
                        <button
                          @click="editUserData(item), btn_update_active=1, modal_title='Editar Usuario'"
                          data-bs-toggle="modal"
                          data-bs-target="#exampleModal"
                          data-bs-original-title=""
            ...
            <div class="row mt-4 align-items-center">
              <div class="col-md-6">
                  <p class="card-text">Mostrando de {{(users.from)}} a {{(users.to)}} de {{users.total}} usuarios.</p>
              </div>             
              <div class="col-md-6">
                <nav>
                  <ul class="pagination justify-content-end pagination-primary">
                      <li   :class="(users.current_page==1)?'page-item disabled':'page-item'" ><a @click.prevent="loadUser(users.current_page-1)" href="" tabindex="-1" data-bs-original-title="" title="" class="page-link">Anterior</a></li>
                      <li v-if="users.current_page>1" class="page-item"><a @click.prevent="loadUser(users.current_page-1)" href="" data-bs-original-title="" title="" class="page-link">{{users.current_page-1}}</a></li>
                      <li class="page-item active">
                          <a href="#" data-bs-original-title="" title="" class="page-link">{{users.current_page}}<span class="sr-only">(current)</span></a>
                      </li>
                      <li v-if="users.current_page!=users.last_page" class="page-item"><a @click.prevent="loadUser(users.current_page+1)" href="" data-bs-original-title="" title="" class="page-link">{{users.current_page+1}}</a></li>
                      <li :class="(users.current_page==users.last_page)?'page-item disabled':'page-item'"><a @click.prevent="loadUser(users.current_page+1)"  href="" data-bs-original-title="" title="" class="page-link">Siguiente</a></li>
                  </ul>
                </nav>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    loadUser: function (page) {
      this.page = page
      axios.get("/api/user/load?page="+this.page,{
        params: {
          buscador: this.buscador,
          buscadorS: this.buscadorS
        }
      }).then((res) => {
        this.users = res.data;
    ...
    createUser: function () {
      axios.post("/api/user/add", this.user).then((res) => {
        Swal.fire({
          ...
          text: "Se registró usuario correctamente.",
          icon: 'success',
    ...
    editUser: function () {
      axios.put("/api/user/edit", this.user).then((res) => {
        Swal.fire({
          ...
          text: "Se actualizó usuario correctamente.",
          icon: 'success',
          confirmButtonColor: '#98B631',
          confirmButtonText: 'De acuerdo'
        })         
    ...
    buscarUsuario() {
      ...
      this.setTimeoutBuscador = setTimeout(this.loadUser, 360)
    }
