            ...
            <div class="row">
              <div class="col-md-4 col-sm-12">
                ...
                <div class="input-group input-group-sm">
                  ...
                  <input class="form-control" type="text" v-model="buscador" @keyup="buscarMarca" placeholder="Nombre de marca...">
                </div>
              </div>

              <div class="col-md-8 col-sm-12 align-self-end text-end">
                <button
                  @click="btn_update_active=0, modal_title='Nueva Marca', provider={}"
                    ...
                    <div class="row">
                      <div class="col-md-12">
                        ...
                        <div class="mb-3">
                          ...
                          />
                          <template v-if="$v.brand.title.$error">
                            <p
                              class="errorMessage error"
                              v-if="!$v.brand.title.required"
                            >
            ...
            <div class="card-body" style="padding: 20px 40px !important">
              <div class="table-responsive style-1 table-hover non-hover">
                <table class="table table-bordered">
                  ...
                  <tbody>
                    <tr style="vertical-align:middle" v-for="item in brands" :key="item.id" >
                      <th>{{ item.id }}</th>
                      <td><img width="50" :src="'images/brand/'+item.image" :alt="item.title"></td>
                      <td>{{ item.title }}</td>
                      <td>
                        <div class="btn-group">
                          <button @click="editBrandData(item), btn_update_active=1, modal_title='Editar Proveedor'" class="btn btn-primary btn-xs" type="button" title=""  data-bs-toggle="modal"  data-bs-target="#exampleModal">
                          ...
                          <button  @click="deleteBrand(item)" class="btn btn-secondary btn-xs" type="button" title="">
              ...
              <div class="row mt-4 align-items-center">
                <div class="col-md-6">
                   <p class="card-text">Mostrando de {{(pagination.from)}} a {{(pagination.to)}} de {{pagination.total}} marcas.</p>
                </div>
                <div class="col-md-6">
                  <nav>
...
export default {
  ...
  methods: {
    //Métodos de carga
    loadBrands:function(page) {
      this.page = page
      axios.get('/api/brand/load?page='+this.page,{
        params: {
    ...
    createBrand:function(){
      const blob = JSON.stringify(this.brand);
      let formData = new FormData();
      formData.append('file', this.attached);
      formData.append("document", blob);
      
      axios.post( '/api/brand/add',formData).then(res=>{
        Swal.fire({
          title: 'Registro Exitoso!',
          text: "Se registró marca correctamente.",
          icon: 'success',
    ...
    editBrand:function() {
      ...
      axios.post( '/api/brand/edit',formData).then(res=>{
        Swal.fire({
          title: 'Actualización Exitosa!',
          text: "Se actualizó marca correctamente.",
          icon: 'success',
          confirmButtonColor: '#98B631',
          confirmButtonText: 'De acuerdo'
        })        
    ...
    deleteBrand:function(item) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            axios.post('/api/brand/delete',this.brand).then(res=>{
              Swal.fire({
                title: 'Eliminado!',
                text: "Se ha eliminado la marca.",
                icon: 'success',
                confirmButtonColor: '#98B631',
                confirmButtonText: 'De acuerdo'
              })   
    ...
    buscarMarca(){
      ...
      this.setTimeoutBuscador = setTimeout(this.loadBrands, 360)
    },    
  },
